...
if(!isset($_SESSION['logged_in'])){
    ...
    if(isset($_POST['place_order'])){
       ...
       if (!preg_match("/([\w\-]+\@[\w\-]+\.[\w\-]+)/",$email)) {
        ...
        header('location: ../checkout.php?error=Enter a Valid Email');
        exit;

        }else if(!preg_match('/^[0-9]{10}+$/', $phone)){
    
        header('location: ../checkout.php?error=Enter a valid Phone Number');
        exit;
        }else{
            ...
            if(isset($_POST['place_order']) ){
                ...
                //5. remove everything from cart --> delay (only after payment is done)
                unset($_SESSION['cart']);
                ...
                //6. inform user whether everything is fine or is there a problem
